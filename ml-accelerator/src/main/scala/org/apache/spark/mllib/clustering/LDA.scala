...
class LDA private (
  ...
  def run(documents: RDD[(Long, Vector)]): LDAModel = {
    ...
    while (iter < maxIterations) {
      ...
      state match {
        case x: OnlineLDAOptimizer => state.next(timer)
        case _ => state.next()
      }
      val elapsedSeconds = (System.nanoTime() - start) / 1e9
      iterationTimes(iter) = elapsedSeconds
      logInfo(s"Iteration ${iter}: ${iterationTimes(iter)}")
      iter += 1
    }
    timer.stop("train")
    timer.stop("Total")
    logInfo("\n=======Time Summary=======\n")
    logInfo(timer.toString())
    logInfo("\n==========================")
    logInfo("\n=======Time Iteration=======\n")
    logInfo(Range(0, maxIterations)
      .map(i => s"Iteration ${i}: ${iterationTimes(i)}").mkString("\n"))
    logInfo("\n==========================")
