...
class SVMKernel {
  case class SVCSchema(label: Int, features: Vector)
  def runJob(spark: SparkSession,params: SVMParams): (Double, Double) = {
    import spark.implicits._
    val sc = spark.sparkContext
    val startTime = System.currentTimeMillis()
    val parsedData = if (params.datasetName == "rcv") {
      spark.read.format("libsvm").option("numFeatures", 47236).load(params.trainingDataPath)
        .withColumn("label", when($"label" === -1.0, 0.0).otherwise($"label"))
        .repartition(params.numPartitions).cache()
    } else if (params.datasetName == "epsilon") {
      spark.read.format("libsvm").option("numFeatures", 2000).load(params.trainingDataPath)
        .withColumn("label", when($"label" === -1.0, 0.0).otherwise($"label"))
        .repartition(params.numPartitions).cache()
    } else {
      spark.read.schema(Encoders.product[SVCSchema].schema).format("orc").load(params.trainingDataPath)
        .repartition(params.numPartitions).cache()
    }
    println(s"trainingData: ${parsedData.count()}")
    ...
    val parsedTest = if (params.datasetName == "rcv") {
      spark.read.format("libsvm").option("numFeatures", 47236).load(params.testDataPath)
        .withColumn("label", when($"label" === -1.0, 0.0).otherwise($"label"))
        .repartition(params.numPartitions).cache()
    } else if (params.datasetName == "epsilon") {
      spark.read.format("libsvm").option("numFeatures", 2000).load(params.testDataPath)
        .withColumn("label", when($"label" === -1.0, 0.0).otherwise($"label"))
        .repartition(params.numPartitions).cache()
    } else {
      spark.read.schema(Encoders.product[SVCSchema].schema).format("orc").load(params.testDataPath)
        .repartition(params.numPartitions).cache()
    }
