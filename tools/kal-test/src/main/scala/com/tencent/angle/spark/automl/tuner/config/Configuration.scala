...
class Configuration(
  ...
  def getParamMapWithParent: ParamMap = {
    val paramMap = ParamMap.empty
    for (name: String <- param2Idx.keys) {
      val nameString = name.split("__")
      val doc = param2Doc(name)
      doc match {
        case "Int" =>
          val param: Param[Int] = new Param(nameString(0), nameString(1), param2Doc.getOrElse(name, ""))
          paramMap.put(param, vector(param2Idx(name)).toInt)
        case _ =>
          val param: Param[Double] = new Param(nameString(0), nameString(1), param2Doc.getOrElse(name, ""))
          paramMap.put(param, vector(param2Idx(name)))
      }
    }
    paramMap
