...
object ParamSpace {
  ...
  def fromConfigString(name: String, config: String, doc: String): ParamSpace[Double] = {
    val vType =
      if (config.trim.startsWith("[") && config.trim.endsWith("]"))
        "continuous"
      else if (config.trim.startsWith("{") && config.trim.endsWith("}"))
    ...
    vType match {
      case "continuous" => ContinuousSpace(name, config, doc)
      case "discrete" => DiscreteSpace[Double](name, config, doc)
      case _ => throw new AutoMLException(s"auto param config is not supported")
    }
  }
