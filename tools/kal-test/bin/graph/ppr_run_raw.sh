...
source conf/graph/ppr/ppr_spark_raw.properties
...
if [ ${api_name} == "fixMS" ]; then
  outputPath="/tmp/graph/result/ppr/${dataset_name}/${api_name}_${src}_raw"
  ...
  echo "start to submit spark jobs--${api_name}_${dataset_name}_${src}_raw"
  spark-submit \
  ...
  ./lib/kal-test_2.11-0.1.jar ${dataset_name} ${api_name} ${num_partitions_val} ${data_path_val} "yes" ${src} ${outputPath} | tee ./log/log
else
  ...
  do
    outputPath="/tmp/graph/result/ppr/${dataset_name}/${api_name}_${source_id}_raw"
    ...
    echo "start to submit spark jobs--${api_name}_${dataset_name}_${source_id}_raw"
    spark-submit \
    --class com.bigdata.graph.PersonalizedPageRankRunner \
    --master yarn \
    --deploy-mode ${deploy_mode_val} \
    ...
    --driver-class-path "lib/kal-test_2.11-0.1.jar:lib/snakeyaml-1.19.jar" \
    --conf "spark.executor.extraClassPath=fastutil-8.3.1.jar" \
    ./lib/kal-test_2.11-0.1.jar ${dataset_name} ${api_name} ${num_partitions_val} ${data_path_val} "yes" ${source_id} ${outputPath} | tee ./log/log
  done
fi
