...
function usage() {
  ...
  echo "1st argument: name of dataset: comlj, orkut, uk_2002, arabic_2005"
...
source conf/graph/slpa/slpa_spark.properties
...
outputPath="/tmp/graph/result/slpa/${dataset_name}/${is_raw}"
...
echo "start to submit spark jobs --- SLPA"
if [ ${is_raw} == "no" ]; then
...
spark-submit \
  --class com.bigdata.graph.SLPARunner \
  --master yarn \
