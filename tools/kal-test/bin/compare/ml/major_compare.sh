...
function usage() {
  ...
  echo "1st argument: algorithm name: als kmeans linr svm dtr(dt.regression) gbdtr(gbdt.regression) rfr(rf.regression) xgbtr(xgbt.regression) lgbmr(lgbmr.regression) bor(bor.regression) fmr(fmr.regression)
                                      lda crf logr svm dtc(dt.classification) gbdtc(gbdt.classification) rfc(rf.classification) xgbtc(xgbt.classification) lgbmc(lgbmc.classification) boc(boc.classification) fmc(fmc.classification)
...
if [ $alg == "als" ] || [ $alg == "kmeans" ]|| [ $alg == "linr" ]|| [ $alg == "svm" ]|| [ $alg == "dtr" ]|| [ $alg == "gbdtr" ]|| [ $alg == "rfr" ]|| [ $alg == "xgbtr" ] || [ $alg == "lgbmr" ] || [ $alg == "bor" ] || [ $alg == "fmr" ]; then
  class_name=com.bigdata.compare.ml.DownEvaluationVerify
elif [ $alg == "logr" ] || [ $alg == "svm" ] || [ $alg == "dtc" ] || [ $alg == "gbdtc" ] || [ $alg == "rfc" ] || [ $alg == "xgbtc" ] || [ $alg == "crf" ] || [ $alg == "lgbmc" ] || [ $alg == "boc" ] || [ $alg == "fmc" ]; then
...
elif [ $alg == "encoder" ] ; then
  class_name=com.bigdata.compare.ml.EncoderVerify
