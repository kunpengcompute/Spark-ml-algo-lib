...
function usage() {
  ...
  echo "1rd argument: name of dataset: [cityu/msr/as]"
...
source conf/ml/crf/crf_spark.properties
dataset_name=$1
is_raw=$2
if_check=$3
...
echo "start to submit spark jobs --- CRF-${model_conf}"
if [ ${is_raw} == "no" ]; then
  ...
  spark-submit \
  --class com.bigdata.ml.CRFRunner \
  ...
  --jars "lib/boostkit-ml-kernel-client_${scala_version_val}-${kal_version_val}-${spark_version_val}.jar" \
  ./lib/kal-test_${scala_version_val}-0.1.jar ${model_conf} ${data_path} ${cpu_name} ${save_resultPath_val} | tee ./log/log
fi