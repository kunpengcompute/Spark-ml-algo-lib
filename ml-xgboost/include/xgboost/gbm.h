...
class GradientBooster : public Model, public Configurable {
 ...
 public:
   ...
   * \param iter defines number of iteration (used for iteration dependent boosting algorithms )
   * \param _last_round_best_loss array of best losses achieved on previous round
   */
  virtual void DoBoost(DMatrix* p_fmat, HostDeviceVector<GradientPair>* in_gpair,
                       PredictionCacheEntry* predt, int iter,
                       float* _last_round_best_loss) = 0;

  /*!
   * \brief same as previous method but without iter and _last_round_best_loss parameters
   */
  void DoBoost(DMatrix* p_fmat, HostDeviceVector<GradientPair>* in_gpair,
               PredictionCacheEntry* predt) {
    this->DoBoost(p_fmat, in_gpair, predt, 0, NULL);
  }
