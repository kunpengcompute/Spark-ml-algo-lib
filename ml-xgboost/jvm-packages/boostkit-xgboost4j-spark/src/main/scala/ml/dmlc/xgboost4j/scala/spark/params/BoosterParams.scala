...
private[spark] trait BoosterParams extends Params {
  ...
  final val growPolicy = new Param[String](this, "growPolicy",
    "Controls a way new nodes are added to the tree. Currently supported only if" +
      " tree_method is set to hist. Choices: depthwise, lossguide, depthwiselossltd. " +
      " depthwise: split at nodes closest to the root. " +
      " lossguide: split at nodes with highest loss change." +
      " depthwiselossltd: nodes that provide low loss change are prevented from split.",
...
private[spark] object BoosterParams {
  ...
  val supportedGrowthPolicies = HashSet("depthwise", "lossguide", "depthwiselossltd")
