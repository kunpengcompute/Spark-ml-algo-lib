...
object XGBoost extends Serializable {
  ...
  private[spark] def trainForNonRanking(
    ...
    if (evalSetsMap.isEmpty) {
      ...
      coPartitionNoGroupSets(trainingData,
      evalSetsMap, xgbExecutionParams.numWorkers)
        .mapPartitions {
    ...
    if (evalSetsMap.isEmpty) {
      trainingData.mapPartitions(labeledPointGroups => {
        ...
        buildDistributedBooster(watches, xgbExecutionParam, rabitEnv,
          xgbExecutionParam.obj, xgbExecutionParam.eval,
          prevBooster)
      }).cache()
    } else {
      coPartitionGroupSets(trainingData,
      evalSetsMap, xgbExecutionParam.numWorkers).mapPartitions(
  ...
  private[spark] def cacheData(ifCacheDataBoolean: Boolean, input: RDD[_]): RDD[_] = {
  ...
  private[spark] def contextTrain(
    ...
    val rabitTracker = XGBoost.startTracker(xgbExecParams.numWorkers,
      xgbExecParams.trackerConf)
    try {
      val parallelismTracker = new SparkParallelismTracker(sc, xgbExecParams.timeoutRequestWorkers,
        xgbExecParams.numWorkers)
      val rabitEnv = rabitTracker.getWorkerEnvs
      rabitEnv.putAll(rabitParams) // adding rabit params

      val boostersAndMetrics = if (hasGroup) {
        XGBoost.trainForRanking(trainingData.left.get, xgbExecParams, rabitEnv, prevBooster,
          evalSetsMap)
      } else {
        XGBoost.trainForNonRanking(trainingData.right.get, xgbExecParams, rabitEnv,
      ...
      sparkJobThread.setUncaughtExceptionHandler(rabitTracker)
      sparkJobThread.start()

      // wait for tracker
      val trackerReturnVal = parallelismTracker.execute(rabitTracker.waitFor(0L))
  ...
  private[spark] def trainDistributed(
    ...
    val rabitParams = xgbParamsFactory.buildRabitParams.asJava

    val ifCacheDataBoolean = XGBoostUtil.checkIfCacheData(xgbExecParams.cacheTrainingSet, params)

    val transformedTrainingData = composeInputData(trainingData, ifCacheDataBoolean,
    ...
    try {
      val (booster, metrics) = XGBoostUtil.trainDistributedKernel(params,
        xgbExecParams, rabitParams, hasGroup, sc, transformedTrainingData,
        evalSetsMap, prevBooster)
      ...
      uncacheTrainingData(ifCacheDataBoolean, transformedTrainingData)
    }
  }

  private[spark] def uncacheTrainingData(
      ifCacheDataBoolean: Boolean,
      transformedTrainingData: Either[RDD[Array[XGBLabeledPoint]], RDD[XGBLabeledPoint]]): Unit = {
    if (ifCacheDataBoolean) {
