...
public class Booster implements Serializable, KryoSerializable {
   ...
   * @param dtrain training data
   * @param iter   current iteration number
   * @param last_round_best_loss last round best loss
   * @throws XGBoostError native error
   */
  public void update(DMatrix dtrain, int iter, float[] last_round_best_loss) throws XGBoostError {
    XGBoostJNI.checkCall(XGBoostJNI.XGBoosterUpdateOneIterLtd(handle, iter, dtrain.getHandle(),
            last_round_best_loss));
  }
