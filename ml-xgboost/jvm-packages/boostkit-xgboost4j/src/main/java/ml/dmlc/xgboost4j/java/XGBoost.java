...
public class XGBoost {
  ...
  public static Booster trainAndSaveCheckpoint(
    ...
    Integer num_class = tryGetIntFromObject(params.get("num_class"));
    float[] last_round_best_loss = new float[num_class == null ? 1 : num_class];
    ...
    for (int iter = booster.getVersion() / 2; iter < numRounds; iter++) {
      if (booster.getVersion() % 2 == 0) {
        if (obj != null) {
          ...
          booster.update(dtrain, iter, last_round_best_loss);
        }
