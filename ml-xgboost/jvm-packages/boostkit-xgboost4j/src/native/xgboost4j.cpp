 ...
 * Signature: (JIJ)V
 */
JNIEXPORT jint JNICALL Java_ml_dmlc_xgboost4j_java_XGBoostJNI_XGBoosterUpdateOneIterLtd
  (JNIEnv *jenv, jclass jcls, jlong jhandle, jint jiter, jlong jdtrain,
    jfloatArray j_last_round_best_loss) {
  BoosterHandle handle = (BoosterHandle) jhandle;
  DMatrixHandle dtrain = (DMatrixHandle) jdtrain;

  jfloat* last_round_best_loss = jenv->GetFloatArrayElements(j_last_round_best_loss, 0);
  int ret = XGBoosterUpdateOneIterLtd(handle, jiter, dtrain, last_round_best_loss);
  JVM_CHECK_CALL(ret);
  jenv->ReleaseFloatArrayElements(j_last_round_best_loss, last_round_best_loss, 0); 
