...
void QuantileHistMaker::Update(HostDeviceVector<GradientPair> *gpair,
  ...
  for (auto tree : trees) {
    builder_->Update(gmat_, gmatb_, column_matrix_, gpair, dmat, tree, iter, p_last_round_best_loss);
  }
...
bool QuantileHistMaker::UpdatePredictionCache(
    const DMatrix* data,
    HostDeviceVector<bst_float>* out_preds, int iter) {
  // don't save cache
  if (SaveCacheChecker(param_.sampling_strategy, param_.subsample, iter, param_.sampling_step)) {
    // save cache
...
void QuantileHistMaker::Builder::AddSplitsToTree(
  ...
  for (auto const& entry : qexpand_depth_wise_) {
    ...
    } else if ((param_.grow_policy == TrainParam::kDepthWiseLossLtd) &&
        KDepthWiseLossLtdLeafChecker(param_.min_loss_ratio,
            *p_last_round_best_loss, snode_[nid].best.loss_chg)) {
      (*p_tree)[nid].SetLeaf(snode_[nid].weight * param_.learning_rate);
    } else {
...
void QuantileHistMaker::Builder::EvaluateAndApplySplits(
    ...
    int depth,
    unsigned *timestamp,
    std::vector<ExpandEntry> *temp_qexpand_depth, int _iter, float* p_last_round_best_loss) {
  EvaluateSplits(qexpand_depth_wise_, gmat, hist_, *p_tree, _iter);

  std::vector<ExpandEntry> nodes_for_apply_split;
  AddSplitsToTree(gmat, p_tree, num_leaves, depth, timestamp,
                  &nodes_for_apply_split, temp_qexpand_depth, p_last_round_best_loss);
...
void QuantileHistMaker::Builder::ExpandWithDepthWise(
  ...
  for (int depth = 0; depth < param_.max_depth + 1; depth++) {
    ...
    EvaluateAndApplySplits(gmat, column_matrix, p_tree, &num_leaves, depth, &timestamp,
                   &temp_qexpand_depth, _iter, p_last_round_best_loss);
    // clean up
  ...
  // update best_split_loss_chg
  if (p_last_round_best_loss != nullptr) {
    size_t size_ = snode_.size();
    auto* nodes_best_loss_chg = new bst_float[size_];
    for (size_t i = 0; i < size_; i++) {
      nodes_best_loss_chg[i] = snode_[i].best.loss_chg;
    }
    UpdateBestSplitLossChange(*p_last_round_best_loss, size_, *nodes_best_loss_chg);
    delete[] nodes_best_loss_chg;
  }
}
...
void QuantileHistMaker::Builder::ExpandWithLossGuide(
  ...
  this->EvaluateSplits({node}, gmat, hist_, *p_tree, _iter);
  ...
  while (!qexpand_loss_guided_->empty()) {
    ...
    if (candidate.IsValid(param_, num_leaves)) {
      ...
      this->EvaluateSplits({left_node, right_node}, gmat, hist_, *p_tree, _iter);
...
void QuantileHistMaker::Builder::Update(const GHistIndexMatrix& gmat,
  ...
  this->InitData(gmat, gpair_h, *p_fmat, *p_tree, iter);

  if (param_.grow_policy == TrainParam::kLossGuide) {
    ExpandWithLossGuide(gmat, gmatb, column_matrix, p_fmat, p_tree, gpair_h, iter);
  } else {
    ExpandWithDepthWise(gmat, gmatb, column_matrix, p_fmat, p_tree, gpair_h, iter, p_last_round_best_loss);
  }
  ...
  pruner_->Update(gpair, p_fmat, std::vector<RegTree*>{p_tree}, iter, p_last_round_best_loss);
...
void InitSampling(const std::vector<GradientPair>& gpair,
...
#else
  float threshold_grad_val = goss ? ThresholdGradientCal(gpair, rnd, subsample) :
  ...
  #pragma omp parallel num_threads(nthread)
  {
    ...
    if (enable_bbgen) {
      struct BbgenSamplingParam bbgenParam = {.subsample = subsample, .tid = tid,
          .discardSize = discard_size, .ibegin = ibegin, .iend = iend,
          .thresholdGradVal = threshold_grad_val};
      BbgenSampling(bbgenParam, rnds, gpair, *p_row_indices, row_offsets);
    } else {
      std::bernoulli_distribution coin_flip(subsample);
      ...
      if (goss) {
        for (size_t i = ibegin; i < iend; ++i) {
          if (gpair[i].GetHess() >= 0.0f &&
            ((fabs(gpair[i].GetGrad()) > threshold_grad_val) || coin_flip(rnds[tid]))) {
...
void QuantileHistMaker::Builder::InitData(const GHistIndexMatrix& gmat,
  ...
  if (param_.grow_policy == TrainParam::kDepthWise || param_.grow_policy == TrainParam::kDepthWiseLossLtd) {
    CHECK(param_.max_depth > 0) << "max_depth cannot be 0 (unlimited) "
                                << "when grow_policy is depthwise or depthwiselossltd.";
  }
...
#pragma omp parallel
    ...
    if ((param_.subsample < 1.0f) || (param_.sampling_step > 1)) {
      CHECK_EQ(param_.sampling_method, TrainParam::kUniform)
        << "Only uniform sampling is supported, "
        << "gradient-based sampling is only support by GPU Hist.";
      struct InitSamplingKernelParam initParam = {.nthread = (size_t)this->nthread_, .iter = iter,
          .lastIterSampling = &last_iter_sampling, .samplingStrategy = param_.sampling_strategy,
          .samplingStep = param_.sampling_step, .enableBbgen = param_.enable_bbgen, .subsample = param_.subsample};
      InitSamplingKernel(gpair, fmat, row_indices, last_sampling, initParam);
    } else {
...
void QuantileHistMaker::Builder::EvaluateSplits(const std::vector<ExpandEntry>& nodes_set,
  ...
  std::vector<int> proc_grid_idx(n_nodes_in_set * 2);
  if (param_.random_split_denom > 1) {
    for (auto& e : proc_grid_idx) {
      e = _iter;
    }
  }
  common::ParallelFor2d(space, this->nthread_, [&](size_t nid_in_set, common::Range1d r) {
    ...
    for (auto idx_in_feature_set = r.begin(); idx_in_feature_set < r.end(); ++idx_in_feature_set) {
      ...
      if (interaction_constraints_.Query(nid, fid)) {
        if (param_.default_direction == TrainParamKernel::learn) {
          auto grad_stats = this->EnumerateSplit<+1>(gmat, node_hist, snode_[nid],
              &best_split_tloc_[nthread*nid_in_set + tid], fid, nid, 
              (param_.random_split_denom > 1 ? proc_grid_idx[2 * nid_in_set] : 0));
          if (SplitContainsMissingValues(grad_stats, snode_[nid])) {
            this->EnumerateSplit<-1>(gmat, node_hist, snode_[nid],
                &best_split_tloc_[nthread*nid_in_set + tid], fid, nid,
                (param_.random_split_denom > 1 ? proc_grid_idx[2 * nid_in_set + 1] : 0));
          }
        } else if (param_.default_direction == TrainParamKernel::right) {
          auto grad_stats = this->EnumerateSplit<+1>(gmat, node_hist, snode_[nid],
              &best_split_tloc_[nthread*nid_in_set + tid], fid, nid, 
              (param_.random_split_denom > 1 ? proc_grid_idx[2 * nid_in_set] : 0));
        } else {
...
GradStats QuantileHistMaker::Builder::EnumerateSplit(
    ...
    SplitEntry *p_best, bst_uint fid, bst_uint nodeID, int proc_grid_idx) const {
  ...
  // index of grid that should be processed
  int eff_split_denom = EffSplitDenomCal(param_.random_split_denom, param_.max_bin);
  ...
  for (int32_t i = ibegin; i != iend; i += d_step) {
    ...
    if (e.sum_hess >= param_.min_child_weight) {
      ...
      // skip some splits
      if (!SplitSkipChecker(eff_split_denom, i, proc_grid_idx)) {
