...
struct TrainParam : public XGBoostParameter<TrainParam> {
  ...
  // minimum loss ratio relative to previous round to make split
  float min_loss_ratio;
  ...
  int max_bin;
  // growing policy
  enum TreeGrowPolicy { kDepthWise = 0, kLossGuide = 1, kDepthWiseLossLtd = 2 };
  int grow_policy;
  ...
  // (for GOSS strategy this value also stands for subsample_upper)
  float subsample;
  // sampling method
  ...
  bool enable_bbgen;
  // subsample_lower - effective only for GOSS strategy
  float subsample_lower;
  ...
  DMLC_DECLARE_PARAMETER(TrainParam) {
    ...
    DMLC_DECLARE_FIELD(min_loss_ratio)
        .set_lower_bound(0.0f)
        .set_default(0.0f)
        .describe(
            "Minimum loss ratio; 0 indicates no limit; a limit is for "
            "depthwiselossltd policy");
    ...
    DMLC_DECLARE_FIELD(grow_policy)
        ...
        .add_enum("depthwiselossltd", kDepthWiseLossLtd)
        .describe(
            ...
            "change. (cf. LightGBM). 2: favor splitting at nodes closest to the node, "
            "stopping splitting nodes with low loss improvement");
    ...
    DMLC_DECLARE_FIELD(default_direction)
        .set_default(TrainParamKernel::learn)
        .add_enum("learn", TrainParamKernel::learn)
        .add_enum("left", TrainParamKernel::left)
        .add_enum("right", TrainParamKernel::right)
    ...
    DMLC_DECLARE_FIELD(random_split_denom)
        .set_lower_bound(1)
        .set_default(1)
        .describe("Ratio of split candidates that must be processed (1/r)");
    DMLC_DECLARE_FIELD(cache_opt)
        .set_default(true)
        .describe("EXP Param: Cache aware optimization.");
    DMLC_DECLARE_FIELD(enable_bbgen)
        .set_default(false)
        .describe("flag to enable batch Bernoulli generation");
    DMLC_DECLARE_FIELD(sampling_strategy)
        .set_default(TrainParamKernel::eachTree)
        .add_enum("eachTree", TrainParamKernel::eachTree)
        .add_enum("eachIteration", TrainParamKernel::eachIteration)
        .add_enum("allIteration", TrainParamKernel::allIteration)
        .add_enum("multiIteration", TrainParamKernel::multiIteration)
        .add_enum("gossStyle", TrainParamKernel::gossStyle)
        .describe("eachTree       - new sampling for each tree for each iteration,"
                  "eachIteration  - new sampling each iteration,"
                  "allIteration   - single sampling for all trees for all iterations,"
                  "multiIteration - change sampling per sampling_step iterations,"
                  "gossStyle      - GOSS-like strategy.");
    DMLC_DECLARE_FIELD(sampling_step)
        .set_default(1)
        .describe("Sampling step for sampling_strategies - 3,4");
    DMLC_DECLARE_FIELD(subsample_lower)
        .set_lower_bound(0.0f)
        .set_default(0.2f)
        .describe("Portion of lower gradients");
...
struct SplitEntryContainer {
  ...
  bool NeedReplace(bst_float new_loss_chg, unsigned split_index) const {
    if (std::isinf(new_loss_chg) || std::isnan(new_loss_chg)) {
      // in some cases new_loss_chg can be NaN or Inf, for example when lambda = 0 & min_child_weight = 0
