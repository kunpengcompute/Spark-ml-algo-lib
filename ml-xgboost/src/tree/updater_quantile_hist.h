...
class QuantileHistMaker: public TreeUpdater {
 public:
  ...
  void Update(HostDeviceVector<GradientPair>* gpair,
              DMatrix* dmat,
              const std::vector<RegTree*>& trees, int iter,
              float* p_last_round_best_loss) override;

  bool UpdatePredictionCache(const DMatrix* data,
                             HostDeviceVector<bst_float>* out_preds, int iter) override;
 ...
 protected:
  ...
  struct Builder {
   ...
   protected:
    ...
    virtual void InitData(const GHistIndexMatrix& gmat,
                  const std::vector<GradientPair>& gpair,
                  const DMatrix& fmat,
                  const RegTree& tree, int iter = 0);
    ...
    GradStats EnumerateSplit(const GHistIndexMatrix &gmat, const GHistRow &hist,
                             ...
                             bst_uint fid, bst_uint nodeID, int proc_grid_idx) const;
    ...
    void ExpandWithDepthWise(const GHistIndexMatrix &gmat,
                             ...
                             DMatrix *p_fmat,
                             RegTree *p_tree,
                             const std::vector<GradientPair> &gpair_h, int _iter,
                             float* p_last_round_best_loss);
    ...
    int last_iter_sampling;
    std::vector<size_t> last_sampling;
