...
/*! buffer size used for storing best loss reductions 
 (used for depthwiselossltd grow policy) */
...
void CLITrain(const CLIParam& param) {
  ...
  float last_round_best_loss[BUFF_SIZE];
  int nclass = 1;  // for binary classification nclass should be equal to 1
  for (int i = 0; i < param.cfg.size(); i++) {
    if (param.cfg[i].first == "num_class") {
      nclass = std::stoi(param.cfg[i].second);
    }
  }
  for (int i = 0; i < nclass; i++) {
    last_round_best_loss[i] = 0;
  }
  ...
  for (int i = version / 2; i < param.num_round; ++i) {
    ...
    if (version % 2 == 0) {
      ...
      learner->UpdateOneIter(i, dtrain, last_round_best_loss);
