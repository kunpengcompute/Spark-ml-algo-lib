...
void GBTree::DoBoost(DMatrix* p_fmat,
                     ...
                     PredictionCacheEntry* predt, int iter,
                     float* _last_round_best_loss) {
  ...
  if (ngroup == 1) {
    ...
    float* p_last_round_best_loss = nullptr;
    if (_last_round_best_loss != nullptr) {
      p_last_round_best_loss = _last_round_best_loss;
    }
    BoostNewTrees(in_gpair, p_fmat, 0, &ret, iter, p_last_round_best_loss);
...
#pragma omp parallel for schedule(static)
      ...
      float* p_last_round_best_loss = nullptr;
      if (_last_round_best_loss != nullptr) {
        p_last_round_best_loss = _last_round_best_loss + gid;
      }
      BoostNewTrees(&tmp, p_fmat, gid, &ret, iter, p_last_round_best_loss);
      new_trees.push_back(std::move(ret));
    }
  }
  monitor_.Stop("BoostNewTrees");
  this->CommitModel(std::move(new_trees), p_fmat, predt, iter);
}
...
void GBTree::BoostNewTrees(HostDeviceVector<GradientPair>* gpair,
  ...
  CHECK_EQ(gpair->Size(), p_fmat->Info().num_row_)
      << "Mismatching size between number of rows from input data and size of "
         "gradient vector.";
  for (auto& up : updaters_) {
    up->Update(gpair, p_fmat, new_trees, iter, p_last_round_best_loss);
  }
}

void GBTree::CommitModel(std::vector<std::vector<std::unique_ptr<RegTree>>>&& new_trees,
                         DMatrix* m,
                         PredictionCacheEntry* predts, int iter) {
  ...
  if (model_.learner_model_param->num_output_group == 1 &&
      ...
      num_new_trees == 1 &&
      out->Size() > 0 &&
      updaters_.back()->UpdatePredictionCache(m, out, iter)) {
...
#pragma omp parallel for schedule(static)
  ...
  CommitModel(std::vector<std::vector<std::unique_ptr<RegTree>>>&& new_trees,
              DMatrix* m,
              PredictionCacheEntry* predts, int iter) override {
    int num_new_trees = 0;
    for (uint32_t gid = 0; gid < model_.learner_model_param->num_output_group; ++gid) {
      num_new_trees += new_trees[gid].size();
      model_.CommitModel(std::move(new_trees[gid]), gid);
