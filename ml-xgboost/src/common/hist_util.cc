...
void BuildHistDenseKernel<double, false, uint8_t>(const std::vector<GradientPair>& gpair,
                                                  ...
                                                  const size_t n_features,
                                                  GHistRow hist) {
  const size_t size_dense_false = row_indices.Size();
  const size_t* rid_dense_false = row_indices.begin;
  const float* pgh_dense_false = reinterpret_cast<const float*>(gpair.data());
  const uint8_t* gradient_index_dense_false = gmat.index.data<uint8_t>();
  const uint32_t* offsets_dense_false = gmat.index.Offset();
  double* hist_data_dense_false = reinterpret_cast<double*>(hist.data());
  const uint32_t two{ 2 };  
  alignas(ALIGNAS_NUM) double tmp[2];

#ifdef USE_INTEL_INTRINSICS
  for (size_t i = 0; i < size_dense_false; ++i) {
    const size_t icol_start = rid_dense_false[i] * n_features;
    const size_t idx_gh = two * rid_dense_false[i];
    tmp[0] = pgh_dense_false[idx_gh];
    tmp[1] = pgh_dense_false[idx_gh + 1];
    __m128d v = _mm_load_pd(tmp);

    const uint8_t* gr_index_local = gradient_index_dense_false + icol_start;
    for (size_t j = 0; j < n_features; ++j) {
      const uint32_t idx_bin = two * (static_cast<uint32_t>(gr_index_local[j]) +
        offsets_dense_false[j]);
      __m128d u = _mm_loadu_pd(&(hist_data_dense_false[idx_bin]));
      __m128d res = _mm_add_pd(u, v);
      _mm_storeu_pd(&(hist_data_dense_false[idx_bin]), res);
    }
  }
#endif
#ifdef USE_ARM_INTRINSICS
  for (size_t i = 0; i < size_dense_false; ++i) {
    ...
    tmp[0] = pgh_dense_false[idx_gh];
    tmp[1] = pgh_dense_false[idx_gh + 1];
    float64x2_t v = vld1q_f64(tmp);

    const uint8_t* gr_index_local = gradient_index_dense_false + icol_start;
    for (size_t j = 0; j < n_features; ++j) {
      const uint32_t idx_bin = two * (static_cast<uint32_t>(gr_index_local[j]) +
        offsets_dense_false[j]);
      float64x2_t u = vld1q_f64(&(hist_data_dense_false[idx_bin]));
      float64x2_t res = vaddq_f64(u, v);
      vst1q_f64(&(hist_data_dense_false[idx_bin]), res);
    }
  }
#endif
}
...
template<>
void BuildHistDenseKernel<double, true, uint8_t>(const std::vector<GradientPair>& gpair,
                                                 const RowSetCollection::Elem row_indices,
                                                 const GHistIndexMatrix& gmat,
                                                 const size_t n_features,
                                                 GHistRow hist) {
  const size_t size_dense_true = row_indices.Size();
  const size_t* rid_dense_true = row_indices.begin;
  const float* pgh_dense_true = reinterpret_cast<const float*>(gpair.data());
  const uint8_t* gradient_index_dense_true = gmat.index.data<uint8_t>();
  const uint32_t* offsets_dense_true = gmat.index.Offset();
  double* hist_data_dense_true = reinterpret_cast<double*>(hist.data());
  const uint32_t two{ 2 }; 
  alignas(ALIGNAS_NUM) double tmp[2];

  for (size_t i = 0; i < size_dense_true; ++i) {
    const size_t icol_start = rid_dense_true[i] * n_features;
    const size_t idx_gh = two * rid_dense_true[i];
    tmp[0] = pgh_dense_true[idx_gh];
    tmp[1] = pgh_dense_true[idx_gh + 1];
#ifdef USE_INTEL_INTRINSICS
    __m128d v = _mm_load_pd(tmp);
#endif
#ifdef USE_ARM_INTRINSICS
    ...
    const size_t icol_start_prefetch = rid_dense_true[i + Prefetch::kPrefetchOffset] * n_features;
    PREFETCH_READ_T0(pgh_dense_true + two * rid_dense_true[i + Prefetch::kPrefetchOffset]);
    for (size_t j = icol_start_prefetch; j < icol_start_prefetch + n_features;
      j += Prefetch::GetPrefetchStep<uint8_t>()) {
      PREFETCH_READ_T0(gradient_index_dense_true + j);
    }

    const uint8_t* gr_index_local = gradient_index_dense_true + icol_start;
    for (size_t j = 0; j < n_features; ++j) {
      const uint32_t idx_bin = two * (static_cast<uint32_t>(gr_index_local[j]) +
        offsets_dense_true[j]);
#ifdef USE_INTEL_INTRINSICS
      __m128d u = _mm_loadu_pd(&(hist_data_dense_true[idx_bin]));
      __m128d res = _mm_add_pd(u, v);
      _mm_storeu_pd(&(hist_data_dense_true[idx_bin]), res);
#endif
#ifdef USE_ARM_INTRINSICS
      float64x2_t u = vld1q_f64(&(hist_data_dense_true[idx_bin]));
      float64x2_t res = vaddq_f64(u, v);
      vst1q_f64(&(hist_data_dense_true[idx_bin]), res);
#endif
    }
  }
}
...
template<>
void BuildHistSparseKernel<double, false>(const std::vector<GradientPair>& gpair,
                                          const RowSetCollection::Elem row_indices,
                                          const GHistIndexMatrix& gmat,
                                          GHistRow hist) {
  const size_t size_sparse_false = row_indices.Size();
  const size_t* rid_sparse_false = row_indices.begin;
  const float* pgh_sparse_false = reinterpret_cast<const float*>(gpair.data());
  const uint32_t* gradient_index_sparse_false = gmat.index.data<uint32_t>();
  const size_t* row_ptr_sparse_false = gmat.row_ptr.data();
  double* hist_data_sparse_false = reinterpret_cast<double*>(hist.data());
  const uint32_t two{ 2 };
  alignas(ALIGNAS_NUM) double tmp[2];

#ifdef USE_INTEL_INTRINSICS
  for (size_t i = 0; i < size_sparse_false; ++i) {
    const size_t icol_start = row_ptr_sparse_false[rid_sparse_false[i]];
    const size_t icol_end = row_ptr_sparse_false[rid_sparse_false[i] + 1];
    const size_t idx_gh = two * rid_sparse_false[i];
    tmp[0] = pgh_sparse_false[idx_gh];
    tmp[1] = pgh_sparse_false[idx_gh + 1];
    __m128d v = _mm_load_pd(tmp);

    for (size_t j = icol_start; j < icol_end; ++j) {
      const uint32_t idx_bin = two * gradient_index_sparse_false[j];
      __m128d u = _mm_loadu_pd(&(hist_data_sparse_false[idx_bin]));
      __m128d res = _mm_add_pd(u, v);
      _mm_storeu_pd(&(hist_data_sparse_false[idx_bin]), res);
    }
  }
#endif
#ifdef USE_ARM_INTRINSICS
  for (size_t i = 0; i < size_sparse_false; ++i) {
    ...
    for (size_t j = icol_start; j < icol_end; ++j) {
      const uint32_t idx_bin = two * gradient_index_sparse_false[j];
      float64x2_t u = vld1q_f64(&(hist_data_sparse_false[idx_bin]));
      float64x2_t res = vaddq_f64(u, v);
      vst1q_f64(&(hist_data_sparse_false[idx_bin]), res);
    }
  }
#endif
}
...
template<>
void BuildHistSparseKernel<double, true>(const std::vector<GradientPair>& gpair,
                                         const RowSetCollection::Elem row_indices,
                                         const GHistIndexMatrix& gmat,
                                         GHistRow hist) {
  const size_t size_sparse_true = row_indices.Size();
  const size_t* rid_sparse_true = row_indices.begin;
  const float* pgh_sparse_true = reinterpret_cast<const float*>(gpair.data());
  const uint32_t* gradient_index_sparse_true = gmat.index.data<uint32_t>();
  const size_t* row_ptr_sparse_true = gmat.row_ptr.data();
  double* hist_data_sparse_true = reinterpret_cast<double*>(hist.data());
  const uint32_t two{ 2 };
  alignas(ALIGNAS_NUM) double tmp[2];

  for (size_t i = 0; i < size_sparse_true; ++i) {
    const size_t icol_start = row_ptr_sparse_true[rid_sparse_true[i]];
    const size_t icol_end = row_ptr_sparse_true[rid_sparse_true[i] + 1];
    const size_t idx_gh = two * rid_sparse_true[i];
    tmp[0] = pgh_sparse_true[idx_gh];
    tmp[1] = pgh_sparse_true[idx_gh + 1];
#ifdef USE_INTEL_INTRINSICS
    __m128d v = _mm_load_pd(tmp);
#endif
#ifdef USE_ARM_INTRINSICS
    ...
    const size_t icol_start_prftch = row_ptr_sparse_true[rid_sparse_true[i + Prefetch::kPrefetchOffset]];
    const size_t icol_end_prefect = row_ptr_sparse_true[rid_sparse_true[i + Prefetch::kPrefetchOffset] + 1];
    PREFETCH_READ_T0(pgh_sparse_true + two * rid_sparse_true[i + Prefetch::kPrefetchOffset]);
    for (size_t j = icol_start_prftch; j < icol_end_prefect;
      j += Prefetch::GetPrefetchStep<uint32_t>()) {
      PREFETCH_READ_T0(gradient_index_sparse_true + j);
    }

    for (size_t j = icol_start; j < icol_end; ++j) {
      const uint32_t idx_bin = two * gradient_index_sparse_true[j];
#ifdef USE_INTEL_INTRINSICS
      __m128d u = _mm_loadu_pd(&(hist_data_sparse_true[idx_bin]));
      __m128d res = _mm_add_pd(u, v);
      _mm_storeu_pd(&(hist_data_sparse_true[idx_bin]), res);
#endif
#ifdef USE_ARM_INTRINSICS
      float64x2_t u = vld1q_f64(&(hist_data_sparse_true[idx_bin]));
      float64x2_t res = vaddq_f64(u, v);
      vst1q_f64(&(hist_data_sparse_true[idx_bin]), res);
#endif
    }
  }
}
