...
XGB_DLL int XGBoosterUpdateOneIter(BoosterHandle handle,
  ...
  bst->UpdateOneIter(iter, *dtr, NULL);
  API_END();
}

XGB_DLL int XGBoosterUpdateOneIterLtd(BoosterHandle handle,
                                      int iter,
                                      DMatrixHandle dtrain,
                                      bst_float* last_round_best_loss) {
  API_BEGIN();
  CHECK_HANDLE();
  auto* bst = static_cast<Learner*>(handle);
  auto *dtr = static_cast<std::shared_ptr<DMatrix>*>(dtrain);

  bst->UpdateOneIter(iter, *dtr, last_round_best_loss);
  API_END();
}
