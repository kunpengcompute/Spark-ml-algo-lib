...
class LearnerImpl : public LearnerIO {
 public:
  ...
  void UpdateOneIter(int iter, std::shared_ptr<DMatrix> train, float* p_last_round_best_loss) override {
    ...
    gbm_->DoBoost(train.get(), &gpair_, &predt, iter, p_last_round_best_loss);
  ...
  void BoostOneIter(int iter, std::shared_ptr<DMatrix> train,
    ...
    gbm_->DoBoost(train.get(), in_gpair, &cache_.Entry(train.get()), iter, p_last_round_best_loss);
