...
template<>
inline void Reducer<Sum, float>(const void *src_, void *dst_, int len, const MPI::Datatype &dtype) {
    const auto *src = (const float *)src_;
    auto *dst = (float *)dst_;
    ReducerSumFloat(*src, *dst, len, dtype);
}

template<>
inline void Reducer<Sum, double>(const void *src_, void *dst_, int len, const MPI::Datatype &dtype) {
    const auto *src = (const double *)src_;
    auto *dst = (double *)dst_;
    ReducerSumDouble(*src, *dst, len, dtype);
}
}  // namespace op
